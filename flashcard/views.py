...
def novo_flashcard(request):
    print(request.user)# pode-se usar META, user

    #verificar se o usuario está logado
    if not request.user.is_authenticated:
        return redirect('/usuarios/logar')
    
    if request.method == 'GET':# verificar se a requisicao foi via navegador
        categorias = Categoria.objects.all()
        dificuldades = Flashcard.DIFICULDADE_CHOICES
        flashcards = Flashcard.objects.filter(user=request.user)#listar todos os flashcards do usuario

        #get do filtro de flashcard
...
def deletar_flashcard(request, id):
    #Fazer validação de segurança para não permitir o usuario inspecionar e deletar um outro flashcard
...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        #Buscar no banco os flashcards para o desafio
        flashcards = (
        ...
        if flashcards.count() < int(qtd_perguntas):
            #inserir uma mensagem de erro
            messages.add_message(
                request, constants.SUCCESS, 'Quantidade perguntar maior que quantidade de flashcard!'
                )
            return redirect('/flashcard/iniciar_desafio/')

        flashcards = flashcards[: int(qtd_perguntas)] #slices do python, fatiar a lista do 0 até a quantidade de perguntas
...
def desafio(request, id):
    ...
    if not desafio.user == request.user: #validação de segurança
        raise Http404()
